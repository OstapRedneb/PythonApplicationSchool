...
def GetFirstSolution():
    k = 0
    fileData = open("9.1.txt")
    for s in fileData:
        nums = [int(x) for x in s.split()]
        n4 = [x for x in nums if nums.count(x) == 4]
        n1 = [x for x in nums if nums.count(x) == 1]
        if len(n4) == 4 and len(n1) == 3 and sum(n1) / len(n1) < sum(nums) / len(nums):
            k += 1
    return k


def GetSecondSolution():
    k = 0
    fileData = open("9.2.txt")
    for s in fileData:
        nums = [int(x) for x in s.split()]
        nums.sort()
        n1 = [x for x in nums if nums.count(x) == 1]
        if len(n1) == 5 and (max(nums) + min(nums)) < (nums[1] + nums[2] + nums[3]):
            k += 1
    return k


## Неверное. Не могу понять ошибку
def GetThirdSolution():
    k = 0
    fileData = open("9.3.txt")
    for s in fileData:
        nums = [int(x) for x in s.split()]
        other = [x**2 for x in nums if nums.count(x) != 1]
        n1 = [x for x in nums if nums.count(x) == 1]
        if (len(n1) == 4 and nums.count(((min(nums) + max(nums)) / 2) // 1) != 0) + (len(n1) != 0 and sum(other) < sum([x**2 for x in n1])) == 1:
            k += 1
    return k


def GetFourthSolution():
    k = 0
    fileData = open("9.4.txt")
    for s in fileData:
        nums = [int(x) for x in s.split()]
        ...
        if len(n2) == 2 and len(n1) == 4 and abs(sum(minus)) > sum(plus):
            k += 1
    return k


print(f"Solution 1: {GetFirstSolution()}")
print(f"Solution 2: {GetSecondSolution()}")
print(f"Solution 3: {GetThirdSolution()}")
print(f"Solution 4: {GetFourthSolution()}")